      ...
      >
        <q-card-section v-if="view == 'header'" key="header">
          <q-item>
            ...
            <q-item-section>
              <q-item-label class="text-weight-light text-h5">{{ account }}</q-item-label>
          ...
          <div class="row justify-end items-center">
            <q-btn
              @click="
                openURL(
                  getSelectedBlockExplorer.base +
                    getSelectedBlockExplorer.account +
                    account
                )
              "
              class="q-mr-xs"
            ...
            <date-string
              v-if="is_member"
              prepend="Member since:"
              :date="is_member.member_since"
            />
            <div v-else>Not yet a member</div>
          </div>
        </q-card-section>

        <q-card-section v-else-if="view == 'more'" key="more">
          <div class="q-mb-md">
            <profile-pic
              :size="36"
              iconBackground="bg-primary"
          ...
          <div class="q-gutter-md">
            ...
            <q-btn-dropdown
              ...
              color="primary"
              :label="
                getRequireUsertermsAgreement
                  ? 'you have not agreed the latest userterms'
                  : `Agreed v${getIsMember.agreed_userterms_version}`
              "
              :split="true"
            ...
            >
              <q-list class="primary-hover-list">
                <q-item
                  v-if="getRequireUsertermsAgreement"
                  clickable
                  v-close-popup
                  @click="signuserterms(true)"
                >
                  <q-item-section>
                    <q-item-label
                      >Agree userterms v{{ getLatestUserterms.id }}</q-item-label
                    >
                  </q-item-section>
                </q-item>
                <q-item v-else clickable v-close-popup @click="signuserterms(false)">
                  <q-item-section>
                    <q-item-label
                      >Disagree userterms v{{ getLatestUserterms.id }}</q-item-label
                    >
                  </q-item-section>
                </q-item>
                <q-item clickable v-close-popup @click="userterms_modal = true">
                  <q-item-section>
                    <q-item-label
                      >View latest userterms v{{ getLatestUserterms.id }}</q-item-label
                    >
                  </q-item-section>
                </q-item>
              </q-list>
  ...
  methods: {
    ...
    async signuserterms(agreed = true) {
      ...
      if (res && res.trxid) {
        ...
        let memberobj = Object.assign(this.getIsMember, {
          agreed_userterms_version: new_version,
        });
