      ...
      >
        <template v-slot:header>
          ...
          <q-item-section>
            ...
            <q-item-label caption>Proposed by {{ proposal.proposer }}</q-item-label>
          </q-item-section>
          <q-item-section side>
            <q-item-label caption>
              Threshold
              <span v-if="proposalstate == 'active'"
                >{{ getThresholdScore.approved_weight }}/{{
                  getThresholdScore.threshold
                }}</span
              >
            </q-item-label>
...
export default defineComponent({
  ...
  computed: {
    ...
    getExpirationStats() {
      ...
      let expiration = new Date(this.proposal.expiration + ".000+00:00").getTime();
      ...
      switch (type) {
        case "approve":
          ...
          r = (await this.push(action))
            ? this.proposal.approvals.push(this.getAccountName)
            : null;
          break;
        case "unapprove":
          ...
          r = await this.push(action);
          if (r) {
            this.proposal.approvals = this.proposal.approvals.filter(
              (pa) => pa != this.getAccountName
            );
          }
          break;
        case "cancel":
