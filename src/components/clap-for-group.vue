    ...
    async transferClaps() {
      let open = {
        ...
        data: {
          ...
          amount: {
            contract: this.getAppConfig.system_token.contract,
            quantity:
              Number(0).toFixed(this.getAppConfig.system_token.precision) +
      ...
      if (res && res.trxid) {
        this.$store.commit(
          "group/setClaps",
          this.getActiveGroupConfig.claps + this.clap_amount
        );
