<template>
  <div>
      ...
      <div class="q-mb-md">
        ...
        >
          ...
          <template v-slot:no-option>
            <q-item>
              <q-item-section class="text-italic text-grey"> No Tokens </q-item-section>
            </q-item>
          </template>
        </q-select>
      </div>
        ...
        >
          <template v-slot:hint>
            <span class="text-grey-8 row" v-if="quantity_input_validated">
              <span
                >{{
                  Number(transfer_action.data.quantity).toFixed(
                    getGroupWallet[selected_token_index].precision
                  )
                }}
                {{ getGroupWallet[selected_token_index].symbol }}</span
              >
    ...
    <div v-else class="text-grey-7 text-weight-light">
      Group doesn't own any tokens. Transfer to "<b>{{ getActiveGroup }}</b
      >" to fund the group.
    </div>
  </div>
  <!-- {{ getGroupWallet }} -->
