...
export default defineComponent({
  ...
  methods: {
    getPaymentToken() {
      return this.getGroupWallet.find(
        (t) => t.contract == this.token_contract && t.symbol == this.symbol
      );
    },
    validateAmount(v) {
      // console.log(v)
  ...
  watch: {
    "action.data.quant": function (newV, oldV) {
      if (this.action.data.quant && this.getPaymentToken()) {
        this.action.data.quant = Number(this.action.data.quant).toFixed(
          this.getPaymentToken().precision
        );
      }
    },
  },
