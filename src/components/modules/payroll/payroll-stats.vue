  ...
  <div v-if="payroll">
    <div class="row justify-between items-center overflow-hidden">
      <div class="row">
        ...
        <div class="q-mr-sm">
          <q-badge>Pay Account</q-badge>
          {{ `${payroll.pay_permission.actor}@${payroll.pay_permission.permission}` }}
        </div>
        <div class="q-mr-sm">
          <q-badge>Total Paid</q-badge> {{ payroll.total_paid.quantity }}
        </div>
        <div class="q-mr-sm">
          <q-badge>Allocated</q-badge> {{ payroll.total_allocated }}
        </div>
          ...
          <q-tooltip class="bg-secondary" :delay="500">
            {{
              has_enough_balance !== false
                ...
                : `Balance insufficient to pay allocated payments, need to top up ${payroll.pay_permission.actor}`
            }}
          </q-tooltip>
        </q-icon>
      </transition>
  ...
  computed: {
    has_enough_balance() {
      ...
      if (this.current_balance) {
        return (
          parseFloat(this.current_balance) - parseFloat(this.payroll.total_allocated) >= 0
        );
      }
    },
  },
