    ...
    <q-card>
      <q-card-section>
        <p>
          ...
          <span v-if="getcandidateStakeConfig.amount > 0"
            >You need to lockup a minimum of {{ getcandidateStakeConfig.quantity }} to be
            eligible for registration.</span
          >
        </p>
        <q-input
          v-if="getcandidateStakeConfig.amount > 0"
          v-model="quantity"
          type="number"
      ...
      candidate {{ getIsCandidate }} stakes: {{ getUserStakes }}
...
export default defineComponent({
  ...
  methods: {
    async regCand() {
      ...
      let dummy = 0;
      dummy =
        dummy.toFixed(this.getcandidateStakeConfig.precision) +
        " " +
        this.getcandidateStakeConfig.symbol;
      ...
      let openstake = {
        ...
        data: {
          ...
          stakeasset: {
            contract: this.getcandidateStakeConfig.contract,
            quantity: dummy,
          },
        },
      };
  ...
  watch: {
    ...
    quantity: function (newV, oldV) {
      if (newV != "") {
        this.quantity = Number(this.quantity).toFixed(
          this.getcandidateStakeConfig.precision
        );
      }
    },
  },
