      ...
      >
        <template v-slot:header>
          <q-item-section avatar>
            <profile-pic
              :size="60"
              :account="candidate.cand"
              :icon="getIsGuardian(candidate.cand) ? 'mdi-star' : ''"
            />
          </q-item-section>
...
export default defineComponent({
  ...
  methods: {
    updateVote(e) {
      ...
      if (
        e &&
        this.getCandidates.filter((c) => c.vote).length ==
          this.getElectionsConfig.max_votes
      ) {
        notifyError({
          message: "Maximum number of votes is " + this.getElectionsConfig.max_votes,
        });
        return;
      } else {
