    ...
    <q-item clickable>
      <q-item-section avatar>
        ...
        >
          ...
          <div v-if="computedNewElectionStats">
            New Election
            {{ secondsToDhms(computedNewElectionStats.time_left / 1000) || `NOW` }}
          </div>
        </q-tooltip>
      </q-item-section>
      <q-item-section>
        <q-item-label class="text-weight-light text-h5 text-grey-7"
          >New Election</q-item-label
        >
        <q-item-label>
          <q-linear-progress
      ...
      <q-item-section side>
        <q-btn
          ...
          color="primary"
          @click="newElection"
          :class="enable_new_election_btn ? 'pulse' : ''"
          :loading="is_transacting"
          :disabled="!enable_new_election_btn"
        />
      </q-item-section>
    </q-item>
    ...
    computedNewElectionStats() {
      ...
      if (this.getElectionsConfig && this.getElectionsState) {
        let res = {
          ...
          last_election_ms: new Date(
            this.getElectionsState.last_election + ".000+00:00"
          ).getTime(),
