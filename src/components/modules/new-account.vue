<template>
  ...
  >
    ...
    >
      <div class="q-mt-sm row q-col-gutter-md">
        <div class="col-xs-12">Account {{ new_account_name }} created</div>
        <div class="col-xs-12 row q-gutter-md">
          <q-btn
            @click="new_account_view = 'newaccount'"
            icon="mdi-arrow-left"
            label="create more"
            color="secondary"
          />
          ...
          <q-btn
            @click="
              $emit('requestLink', { actor: new_account_name, permission: 'active' })
            "
            icon="mdi-link-box"
            label="link as module"
            color="primary"
          />
        </div>
      </div>
  ...
  methods: {
    ...
    async create() {
      ...
      let newaccount = {
        ...
        data: {
          ...
          active: {
            accounts: [
              {
                permission: { actor: this.getActiveGroup, permission: "owner" },
                weight: 1,
              },
            ],
            keys: [],
          ...
          owner: {
            accounts: [
              ...
              {
                permission: { actor: this.getAccountName, permission: "active" },
                weight: 1,
              },
            ],
            keys: [],
      ...
      if (this.include_eosio_code) {
        newaccount.data.active.accounts.push({
          permission: { actor: this.new_account_name, permission: "eosio.code" },
          weight: 1,
        });
      }
      let buyram = {
      ...
      let delegatebw = {
        ...
        data: {
          ...
          receiver: this.new_account_name,
          stake_net_quantity:
            this.net_quantity + ` ${this.getAppConfig.system_token.symbol}`,
          stake_cpu_quantity:
            this.cpu_quantity + ` ${this.getAppConfig.system_token.symbol}`,
