<template>
  ...
  >
    <template v-slot:separator>
      <q-btn icon="compare_arrows" round color="primary" dense size="11px"> </q-btn>
    </template>
    ...
    <template v-slot:after>
      ...
      >
        <q-tab-panel :name="getActiveGroup.toUpperCase()" class="overflow-hidden">
          ...
          <p>This is the main account of the group.</p>
        </q-tab-panel>

        <q-tab-panel name="Core" class="overflow-hidden">
          <page-header title="Core Module" />
          <p>
            This module is enabled by default and can't be turned off. It enables all core
            features of the group account.
          </p>
          <p>You can enable additional functionallity in the "Modules" section.</p>
        ...
        <q-tab-panel name="Update Code" class="overflow-hidden">
          ...
          <code-deployer
            :module="{
              module_name: 'core',
              slave_permission: { actor: getActiveGroup, permission: 'owner' },
              has_contract: true,
            }"
          />
        </q-tab-panel>
