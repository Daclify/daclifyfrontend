<template>
  <div class="q-mr-md" v-if="!notifications_not_supported">
    ...
    <q-btn
      v-if="!browser_notifications_granted || !isSubscribedForTopic"
      @click="enableNotificationsAndGetToken"
      icon="mdi-bell"
      label="enable"
      color="primary"
    ...
    <q-btn
      v-else
      @click="unSubscribeToTopic(getCurrentFCMToken, getActiveGroup)"
      icon="mdi-bell-off"
      label="disable"
      color="primary"
      :loading="api_call_waiting"
  ...
  methods: {
    ...
    async subscribeToTopic(token, topic_name) {
      ...
      let res = await this.$axios.post(
        `${this.getAppConfig.api.url}/subscribe_notifications`,
        {
          token: token,
    ...
    async unSubscribeToTopic(token, topic_name) {
      this.api_call_waiting = true;
      let res = await this.$axios.post(
        `${this.getAppConfig.api.url}/unsubscribe_notifications`,
        {
          token: token,
          topic: `/topics/${topic_name}`,
        }
