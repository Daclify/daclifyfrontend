<template>
  <q-input
    ...
    :rules="[(val) => !!val || '* Required', isHttps, isValidUrl]"
  >
    ...
    <template v-slot:prepend>
      <profile-pic
        :size="40"
        :url="new_avatar"
        @loaded="new_avatar_loaded = true"
        @error="new_avatar_loaded = false"
      />
    </template>
...
export default defineComponent({
  ...
  methods: {
    ...
    async update_avatar() {
      ...
      if (res && res.trxid) {
        this.$store.commit("group/updateAvatar", {
