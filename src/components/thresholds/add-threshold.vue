  ...
  <div class="row q-col-gutter-md q-pb-xs q-mt-md">
      ...
      >
        <template v-slot:hint>
          <div class="text-warning row items-center" v-if="is_existing_threshold">
            ...
            <span
              >Update existing threshold (current:
              {{ is_existing_threshold.threshold }})</span
            >
          </div>
        </template>
      </q-input>
    ...
    <div class="col-xs-12 col-sm-6" v-if="is_existing_threshold">
      <transition
        ...
        tag="div"
      >
        <q-toggle v-model="action.data.remove" left-label label="Delete this threshold" />
      </transition>
    </div>
    ...
    <div class="row justify-between full-width items-center">
      ...
      <propose-bucket-btn
        ...
        :label="is_existing_threshold ? 'update threshold' : 'Add Threshold'"
        :disabled="false"
      />
    </div>
    <!-- <pre>{{action}}</pre> -->
...
export default defineComponent({
  ...
  computed: {
    ...
    is_existing_threshold() {
      ...
      return this.getThresholds.find(
        (t) => t.threshold_name == this.action.data.threshold_name
      );
    },
  },
  methods: {
