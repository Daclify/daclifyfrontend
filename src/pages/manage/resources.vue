  ...
  <q-page padding class="constrain-page-width">
          ...
          <div v-for="item in chunk" :key="item.key" class="col-xs-12 col-sm-6 col-md-4">
            <q-card class="q-pb-md no-shadow border-primary">
              ...
              <q-btn
                ...
                @click="
                  dialog_Resource_warning.type = item.key;
                  dialog_Resource_warning.visible = true;
                "
              />
            </q-card>
          </div>
    ...
    <q-dialog v-model="dialog_Resource_warning.visible">
      <q-card class="overflow-hidden" style="min-width: 350px">
        <q-card-section>
          <page-header
            :title="`${dialog_Resource_warning.type.toUpperCase()} Warning Level`"
          />
          <q-btn
            icon="close"
            flat
          ...
          <p class="text-grey-7 text-caption">
            Set a warning level for the groups
            {{ dialog_Resource_warning.type.toUpperCase() }} consumption.
          </p>
        </q-card-section>
      </q-card>
    </q-dialog>
