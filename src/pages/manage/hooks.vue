<template>
  <q-page padding class="constrain-page-width">
    <div>
      <q-input
        class="q-pb-md"
        placeholder="Find hook"
        outlined
        v-model.trim="searchfilter"
        @input="add_hook_view = false"
      >
      ...
      <q-card>
        <q-toolbar class="bg-secondary text-white shadow-2">
          ...
          <q-btn
            round
            dense
            :icon="add_hook_view ? 'mdi-minus' : 'mdi-plus'"
            color="primary"
        ...
        >
          ...
          >
            ...
            <q-expansion-item
              clickable
              v-for="hook in getFilteredHooks"
              :key="hook.hook_id"
              group="hooks"
            >
    ...
    getFilteredHooks() {
      if (this.getHooks && this.getHooks.length) {
        ...
        if (this.searchfilter) {
          res = res.filter((h) => {
            return (
              h.hook_action_name.includes(this.searchfilter) ||
