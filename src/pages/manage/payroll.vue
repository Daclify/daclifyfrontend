<template>
  <q-page padding class="constrain-page-width">
    ...
    <div v-if="getPayrolls.length">
      ...
      <q-card class="q-pa-sm q-my-md">
        <payroll-stats
          ...
          @onbalance="active_payroll_balance = $event"
        />
      </q-card>
      ...
      <q-card>
        <q-toolbar class="bg-secondary text-white shadow-2">
          ...
          <q-btn
            round
            dense
            :icon="add_payment_view ? 'mdi-minus' : 'mdi-plus'"
            color="primary"
        ...
        >
          <div v-if="!add_payment_view" key="payments">
            <q-list
              v-if="filterPayments.length"
              class="primary-hover-list"
              separator
              striped
            >
              <payment
                v-for="(payment, i) in filterPayments"
                :key="payment.pay_id"
                :payment="payment"
                :class="i % 2 === 0 ? '' : ''"
              />
            ...
            <action-proposer>
              <template v-slot="scope">
                <add-payment
                  ...
                  :payroll="getActivePayRoll"
                  :currentbalance="active_payroll_balance"
                />
              </template>
            </action-proposer>
          </div>
    ...
    <q-card v-else>
      <q-card-section> No payrolls configured yet. </q-card-section>
    </q-card>
  </q-page>
</template>
    ...
    filterPayments() {
      ...
      if (this.searchfilter) {
        payments = payments.filter(
          (p) => p.receiver.includes(this.searchfilter) || p.pay_id == this.searchfilter
        );
      }
      return payments;
    },
